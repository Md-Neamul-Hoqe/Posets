...
/* Re Draw The Posets [Elements & Their Relations] */
...
function PosetreDraw() {
  /* Clear The Canvas */
  PosetIndex.clearRect(0, 0, width, height);
...
function fillOrUnFill(xCoord, yCoord) {
  ...
  if (XYPoints[xCoord][yCoord][2] === false && SELength !== morder) {
    ...
    document.getElementById("ShowErrors").hidden = false;
    document.getElementById("ShowErrors").innerHTML =
      morder + " Elements are already selected.";
    return;
  }
...
function transitionalCoverMatrix() {
  ...
  if (SELength) {
    ...
    for (let j = morder - 3; j >= 0; j--) {
      ...
      if (SEsC[j][2] != undefined && SEsC[j][2] != null && SEsC[j][2].length) {
        // console.log(SEsC),
        //     console.log('j = ', j);
            ...
            if (
              ...
              for (let l = k + 1; l < morder; l++) {
                ...
                if (idxk !== -1) {
                  ...
                  console.log("Copy of Selected Element Matrix:"),
                    console.table(SEsC);
    ...
    let relations = [];
    for (let i = 0; i < SELength; i++) {
      if (SEsC[i][2] != undefined && SEsC[i][2] != null && SEsC[i][2].length) {
...
function DrawingByKeyboard() {
  ...
  Tab = false;
  document.getElementById("IsKeyboard").innerHTML =
    "Keyboard Feature is <span class='text-info'>On</span>. [Use mouse for more flexibility & guidlines]<br> <span class='text-dark'> <kbd>Enter</kbd> to select &amp; <kbd>Shift</kbd> + <kbd>Enter</kbd> to relate</span>";
  ...
  document.onkeyup = function (e) {
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (Shift) {
          ...
          function Select(element) {
            ...
            return (
              element[0] === XYPoints[keyPosY][keyPosX][0] &&
      ...
      default:
        console.log(
          "Please update your browser, if this sit not work perfectly."
        );
    }
...
function resetCanvas() {
  ...
  while ((L = selectedElements.length)) {
    // selectedElements.length dicrease to 0 as deletion
